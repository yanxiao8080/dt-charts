...
export default {
  ...
  methods: {
    ...
    handlerData () {
      ...
      for (const col in columns) {
        ...
        if (isObject(columns[col])) {
          ...
          throw '[DtScatter Function handlerData] columns 不是Object也不是String'
        }
        let seriesItem = {
          // 设置默认配置
          ...this.itemOpt,
      ...
      if (!this.hidden) {
        this.dispatch('DtScatter', 'handleChild', [this.option, this._uid]);
      }
    },
    ...
    hiddenChange () {
      this.hidden ?
        this.dispatch('DtScatter', 'hiddenChild', [this._uid]) :
        this.dispatch('DtScatter', 'handleChild', [this.option, this._uid]);
    },
  }
}